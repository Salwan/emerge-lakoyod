package cmg
{
	...
	{		
		 ...
		 *	</pre>
		 * 
		 * @param the_id To identify this state machine when logging
		 * @param log Do you want to enable state info logging? (errors are always logged)
		*/
		public function StateMachine(the_id:String = "", log:Boolean = false)
		{
			id = the_id;
			bLog = log;
		...
		**/
		public function addState(stateName:String, stateData:Object=null):void
		{
			...
			if ((stateData.enter && stateData.update && stateData.enter === stateData.update) || 
				(stateData.enter && stateData.exit && stateData.enter === stateData.exit) || 
				(stateData.update && stateData.exit && stateData.update === stateData.exit))
			{
				trace("[StateMachine]", id, "Duplicate functions! check state " + stateName);
			}
			_states[stateName] = new State(stateName, stateData.from, stateData.enter, stateData.update, stateData.exit, _states[stateData.parent])
		}
		...
		/**
		 * Updates active state if available.
		 */
		public function update():void
		{
			if (_state in _states)
			{
				if (_states[_state].update)
				{
					_states[_state].update.call(null, null);
				}
			}
		}
		...
		**/
		public function changeState(stateTo:String):void
		{
			...
			if(bLog) trace("[StateMachine]",id,"State Changed to " + _state);
